...
double covfun2(double d, double *cparms){
...
// [[Rcpp::export]]
NumericVector OrderedCompLik3d(NumericVector covparms, NumericVector y,
    ...
    for(i=m; i<n+1; i++){
        ...
        Li[1-1][1-1] = pow( covfun2(0, cparms), -0.5 );
        ...
        for(j=2; j<m+1; j++){  // j = row of Li
            ...
            Li[j-1][j-1] = pow( covfun2(0,cparms) - d, -0.5 );
